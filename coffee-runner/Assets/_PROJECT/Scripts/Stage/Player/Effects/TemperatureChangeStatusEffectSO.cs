...
public class TemperatureChangeStatusEffectSO : StatusUpgradableEffectProperty
{
    ...
    private float GetTemperature(float entitySpeed) => percentage ? (entitySpeed * temperature * 0.01f) : temperature;
    private float GetCondition(float entityTemperature) => percentage ? (entityTemperature * condition * 0.01f) : condition;
